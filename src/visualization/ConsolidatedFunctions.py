...
def get_min_max(bike_data):
    min_lat = bike_data["s_lat"].min()
    max_lat = bike_data["s_lat"].max()
    max_lon = bike_data["s_lng"].max()
    min_lon = bike_data["s_lng"].min()
...
def plot_station_counts(trip_counts, zoom_start=13):
    ...
    for index, row in trip_counts.iterrows():
        ...
        popuptext = popuptext.format(row["StartStation"], row["Departure Count"], row["Arrival Count"], net_departures)
        ...
        folium.CircleMarker(location=(row["s_lat"],
                                      row["s_lng"]),
                            radius=radius,
                            color=color,
                            popup=popup,
...
def get_locations(bike_data):
    locations = bike_data.groupby("s_number").first()
    locations = locations.loc[:, ["StartStation", "s_lat", "s_lng"]]
    return locations
...
def get_trip_counts_by_hour(selected_hour, bike_data):
    ...
    subset_start = bike_data[bike_data["hour_start"] == selected_hour]
    subset_end = bike_data[bike_data["hour_end"] == selected_hour]
